...
using ShopInShop.Interface.Database;
...
namespace ShopInShop.Interface.Services
{
    ...
    {
        private DapperContextHFH _dapperContext;
        private readonly IConfiguration _configuration;
        public TransactionService(IConfiguration configuration)
        {
            _configuration = configuration;
            _dapperContext = new DapperContextHFH(_configuration);
        }
        public async Task<int> GetTransFromCentral(string app, string type, string pathLocal)
        {
            int affecaffectedRows = 0;
            using (IDbConnection conn = _dapperContext.ConnCentralSalesStaging)
            {
                conn.Open();
                try
                {
        ...
        public async Task<List<InvoiceCreated>> GetInvoiceCreatedAsync(List<string> lstOrder)
        {
            ...
            try
            {
                using IDbConnection conn = _dapperContext.ConnEInvoice;
                conn.Open();
